...
loop
{
	...
	jichu_for_select:=get_word(DB,srf_all_Input_trim,"wubi86")
...
if(lm_xl)
{
	jichu_for_select_lianxiang:=get_word_lianxiang(DB,srf_all_Input,"wubi86")
}
...
for index,element in jichu_for_select_Array
{
	...
	{
		jichu_for_select_string.=A_Space . index . ". " . element . "`n> "
		if(maxlen<StrLen(element))
		{
			maxlen:=StrLen(element)
		}
	}
...
jichu_for_select_string:=Trim(jichu_for_select_string,"`n> ")
...
Thisfengefu:=SubStr(fengefu,1,maxlen*4.5)
if (srf_for_select!="")
{
	ToolTip, %srf_all_Input% `n%Thisfengefu%`n%srf_for_select%, A_CaretX + 10 , A_CaretY + 20, 16
